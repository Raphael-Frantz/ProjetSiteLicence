...
<section class="mytitle text-center bg-light">
  <div class="container">
    ...
    <p class="lead mb-0">Retrouvez ici les notes de l'étudiant(e) <span class='badge badge-primary'><?php echo $data['etudiant']; ?></span>.</p>
  </div>
</section>
...
HTML;
        ...
        if(($note['session1'] == 0) && ($note['session2'] >= 0)) {
            ...
            if($note['type'] == InscriptionECModel::TYPE_VALIDE) {
                $msg = "<span class='badge badge-secondary'>{$note['noteip']}/20</span>";
            }
            else {
                if($note['note'] !== null) {
                    $msg = Note::convertirStr($note['note'], $note['max']);
                    if($note['visible'] != 1)
                        $msg .= " <span class='badge badge-secondary'>(cachée)</span>";
                }
                else {
                    $msg = "-";
                }
